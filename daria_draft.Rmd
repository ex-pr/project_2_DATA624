...
#shrinkage=0.01, interaction.depth=7, n.minobsinnode=5, n.trees=1000, RMSE=0.11086, R2=0.5966163, MAE=0.0838286
...
#RMSE=0.11417525, Rsquared=0.54005673, MAE= 0.08642612
...
#size = 10, decay = 0 and bag = FALSE RMSE=0.1152809 R2=0.5613684 MAE=0.08483058
...
#RMSE=0.11742502 R2=0.51488013 MAE=0.08529185
...
#sigma = 0.1 C = 1 RMSE=0.1174607 R2=0.5438569 MAE=0.0864932
...
#RMSE=0.11798682 R2=0.50619395 MAE=0.08546282
...
#Train SVM
svmGrid <- expand.grid(sigma = c(0.001, 0.01, 0.1), C = c(0.1, 1, 10, 100))
svmModel <- train(ph ~ ., data = train_imputed, 
                  method = "svmRadial", 
                  preProc = c("center", "scale"),
                  tuneGrid = svmGrid,
                  trControl = control
...
#sigma = 0.1 C = 1 RMSE=0.1207946 R2=0.520661 MAE=0.08977349
svm_tune <- svmModel$results[svmModel$results$sigma == svmModel$bestTune$sigma & svmModel$results$C == svmModel$bestTune$C, ]
svm_tune

svmPred <- predict(svmModel, newdata = test_imputed)
#RMSE=0.11921656 R2=0.49670669 MAE=0.08745773 
svm_results <- postResample(pred = svmPred, obs = test_imputed$ph)
svm_results
```
...
#nprune = 20 degree = 2 RMSE=0.1270379 R2=0.4689127 MAE=0.09637897
...
#RMSE=0.12849895 R2=0.41981463 MAE=0.09603593 
...
#nprune = 20 degree = 2 RMSE=0.1266452 R2=0.472824 MAE=0.09637897
mars_tune <- marsModel$results[marsModel$results$nprune == marsModel$bestTune$nprune & marsModel$results$degree == marsModel$bestTune$degree, ]
mars_tune

marsPred <- predict(marsModel, newdata = test_imputed)
#RMSE=0.12620060 R2=0.44103613 MAE=0.09603593 
...
set.seed(547) 
xgbGrid <- expand.grid(
  nrounds = seq(100, 500, by = 100),     # Fewer boosting iterations
  eta = c(0.01, 0.1),                   # Fewer learning rates
  max_depth = c(3, 6),                  # Smaller range of tree depths
  gamma = c(0, 1),                      # Fewer gamma values
  colsample_bytree = c(0.8),            # Single value for simplicity
  min_child_weight = c(1, 5),           # Fewer options
  subsample = c(0.8)                    # Single value for simplicity
)
...
write_xlsx(eeval_results, "ph_predictions.xlsx")
```
# 5. Conclusion
