...
Mnf Flow: 
A value of -100 for a large proportion of rows. MAYBE:  
Missing Data: A common placeholder value.  
Domain-Specific Encoding: For example, "no flow" or a specific state in manufacturing.  
Data Error: A mistake in data collection or recording.  
Confirm if -100 is a legitimate value or a placeholder. If it is a placeholder, it should be treated as missing and imputed or removed.  
...
Standardizing column names (make_clean_names()) helps to eliminate problems caused by case sensitivity or spaces in column names, making data manipulation easier and less error-prone.

Identify and remove zero variance predictors (Hyd Pressure 1), which have the same value across all observations and do not help the model differentiate between them. Identifying (nearZeroVar()) and deleting these variables simplifies the modeling process, boosting computing efficiency and potentially increasing model performance by concentrating on more informative features.

The K-Nearest Neighbors imputation method takes advantage of the similarity between observations, filling in missing values using the average (or median) of the 'k' most similar occurrences.Advantages versus MICE: While it can produce comprehensive results via numerous imputations, it is computationally demanding and may bring more variability into the dataset. KNN imputation is simpler and faster than other methods, making it ideal for big datasets with simple relationships.   
Random Forest Imputation of Categorical Variables: It anticipates missing categories by learning decision rules inferred from other factors in the data, allowing for more accurate approximation of missing values. Unlike simpler methods such as mode imputation, which just fills in the most frequent category without taking into account the context provided by other features, this method captures non-linear correlations between features.

The precise handling of 'Mnf Flow' values, which may be considered as placeholders or outliers, demonstrates a data-specific expertise. If '-100' is not a valid manufacturing data point (for example, representing sensor malfunctions or data input errors), marking it as missing helps to avoid biasing the model. KNN imputation was performed after changing these placeholder values to NA to ensure that the replacement values reflect typical operational data and preserve consistency in the manufacturing flow analysis.  

Decision to not adjust outliers initially: Outliers in predictive modeling for manufacturing data can signify genuine fluctuations in the process or crucial anomalies to record. Automatically reducing or changing them without a comprehensive study may result in the loss of important information about the system's behavior under unusual conditions. Depending on the model's final application, recognizing and responding to uncommon conditions may be more beneficial than having a model that can perfectly anticipate regular conditions.

Categorical variables, such as Brand Code, are turned into numerous binary variables, with each signifying the presence (1) or absence (0) of that category. This is critical for models that demand numerical input but do not naturally handle categorical data (such as SVMs and Neural Networks). This encoding broadens the feature space while making the model more adaptable in learning patterns unique to each category.

Interaction terms (such as pc_volume_pressure, which combines pc_volume and carb_pressure) are used to simulate the effects of interactions between two or more variables on pH levels. This is based on domain knowledge, which implies that the combination of these elements may have a non-additive effect on the outcome. Polynomial characteristics (such carb_volume2) enable the model to incorporate non-linear interactions, resulting in a more flexible border in regression models. Binning continuous variables (such as carb_temp_binned) can aid in the robust management of non-linear effects and improve model performance by transforming a continuous variable into a categorical variable that may be easier to model.
...
**Imputing missing values** knn for numeric vars ?

For Brand Code random forest imputation, for the rest - knn.
...
CHECK THAT ALL TRANSFORMATIONS, IMPUTATIONS ARE LEGIT
...
marsModel <- train(ph ~ ., data = train_revised, 
                   method = "earth",
...
#nprune = 20 degree = 2 RMSE=0.1292419 R2=0.4443691 MAE=0.09681492
...
marsPred <- predict(marsModel, newdata = test_revised)
#RMSE=0.1147351 R2=0.5620902 MAE=0.0887774
mars_results <- postResample(pred = marsPred, obs = test_revised$ph)
mars_results
```
...
mnf_flow         100.00
brand_code.C      72.09
pressure_vacuum   54.57
alch_rel          54.16
carb_pressure1    47.91
air_pressurer     47.91
balling           47.91
hyd_pressure3     47.91
setpoint_diff     47.91
...
plot(importance_mars, top = 10, main = "top 10 predictors")
```
...
svmModel <- train(ph ~ ., data = train_revised, 
                  method = "svmRadial", 
                  preProc = c("center", "scale"),
                  tuneGrid = svmGrid,
                  trControl = control
...
#sigma = 0.1 C = 10 RMSE=0.1208973 R2=0.5119372 MAE=0.08873263
svm_tune <- svmModel$results[svmModel$results$sigma == svmModel$bestTune$sigma & svmModel$results$C == svmModel$bestTune$C, ]
svm_tune

svmPred <- predict(svmModel, newdata = test_revised)
#RMSE=0.11363499 R2=0.56862606 MAE=0.08364735 
svm_results <- postResample(pred = svmPred, obs = test_revised$ph)
svm_results
```
...
mnf_flow           0.225134
usage_cont         0.149749
setpoint_diff      0.130429
filler_level       0.098900
carb_flow          0.085364
brand_code.C       0.080026
hyd_pressure_indx  0.076867
carbonation_compos 0.064414
pressure_vacuum    0.049858
fill_pressure      0.042009
brand_code.D       0.036830
oxygen_filler      0.030101
filler_efficiency  0.029419
temperature        0.023827
fill_ounces        0.011828
brand_code.B       0.009413
gas_control_indx   0.009223
pc_volume          0.006079
carb_pressure1     0.005820
pressure_temp      0.005092
...
nnetModel <- train(ph ~ ., data = train_revised, 
                   method = "avNNet",
                   preProc = c("center", "scale"),
                   tuneGrid = nnetGrid,
                   trControl = control,
...
#size = 10, decay = 0 and bag = FALSE RMSE=0.1160184 R2=0.5487825 MAE=0.08579205
...
nnetPred <- predict(nnetModel, newdata = test_revised)
#RMSE=0.10565308 R2=0.62682414 MAE=0.08080004
nnet_results <- postResample(pred = nnetPred, obs = test_revised$ph)
nnet_results
```
...
carb_flow       0.085364
brand_code.C    0.080026
hyd_pressure3   0.054878
pressure_vacuum 0.049858
fill_pressure   0.042009
hyd_pressure2   0.037238
brand_code.D    0.036830
oxygen_filler   0.030101
carb_rel        0.025823
hyd_pressure4   0.025636
temperature     0.023827
alch_rel        0.023403
fill_ounces     0.011828
balling_lvl     0.011025
psc             0.009459
brand_code.B    0.009413
...
#shrinkage=0.01, interaction.depth=7, n.minobsinnode=5, n.trees=1000, RMSE=0.1118648, R2= 0.5853444, MAE=0.0841027
...
#RMSE=0.10193487, Rsquared=0.66519696, MAE= 0.07896032 
...
oxygen_filler        88.27
temperature          84.01
pressure_vacuum      59.90
carb_pressure1       55.74
setpoint_diff        54.87
carb_rel             53.65
filler_speed         49.99
air_pressurer        49.90
...
Residual Plot Analysis for Random Forest: Pattern and Spread. The residuals exhibit no clear pattern of growing or decreasing trends with the fitted values, which is a good indicator. They appear to be randomly distributed around the zero line, indicating that the model does not exhibit heteroscedasticity.
Outliers: Some residuals differ greatly from the rest. These could be outliers in your data or cases when the model fails to predict correctly.

Conclusion: The Random Forest model appears to be working well in terms of handling data volatility with no discernible bias. However, you might want to look into the few outliers to determine if they are caused by data errors or specific occurrences that are difficult to model.

Cross-validation RMSE plot.
RMSE Reduction: The plot demonstrates that RMSE decreases dramatically as the number of predictors in the model increases from one to about ten, after which it plateaus. This means that a subset of the top ten features account for the majority of the predictive power.
Stabilization: The fact that RMSE stabilizes after a certain point indicates that adding more predictors beyond this point does not significantly increase model performance, which is typical of diminishing returns in feature addition.


Conclusion: According to this research, focusing on the top predictors (the number at which the RMSE levels out) may be the most efficient use of data. It promotes a parsimonious model construction method in which adding greater complexity (i.e., more features) does not result in improved performance.


```{r}
# Residual plot for Random Forest
rf_residuals <- residuals(rf_model)
plot(rf_residuals, main = "Residual Plot for Random Forest", ylab = "Residuals", xlab = "Fitted values")

# Cross-Validation Check
set.seed(123)
cv_results <- train(ph ~ ., data = train_new, method = "rf",
