...
Objective: The purpose of this analysis is to understand the beverage manufacturing process at the ABC Beverage company. This is done by completing an expansive evaluation of pH levels, which consists of preliminary data analysis and scrupulous model selection for predicting pH levels in the manufacturing process. Following the discovery of missingness, the group moved to further evaluate the data in a thorough exploratory data analysis.

We began this analysis by looking into which variable would present issues for the model. There was a glaring problem with the state of the original beverage data. Variables Brand Code and MFR were missing 4.5% and 8.2% of data respectively. 
...
Density, Balling, and Carb Rel show strong intercorrelations among themselves. Multicollinearity issues in linear regression models. 
...
One-Hot Encoding: Categorical variables, such as Brand Code, are turned into numerous binary variables, with each signifying the presence (1) or absence (0) of that category. This is critical for models that demand numerical input but do not naturally handle categorical data (such as SVMs and Neural Networks). This encoding broadens the feature space while making the model more adaptable in learning patterns unique to each category.
...
Log and square root transformations were used on various variables, including filler_speed and air_pressurer, to standardize their distributions and reduce skewness.
Transformations are used to reduce variance and make data more normally distributed, which can improve the effectiveness of many statistical learning approaches that rely on normally distributed data.
...
COMPARE THE TRANSFORMED DATA AND ORIGINA TO MAKE SURE THAT OUR TRANSFORMATIONS ARE OK
