...
MARS is a versatile modeling method that captures nonlinearities and variable interactions effectively. Data preparation involves scaling and centering to standardize feature scales. The model is trained using the earth method with a parameter grid specifying degrees and the number of pruning terms (nprune).

MARS builds models by generating piecewise linear splines, useful for handling complex, non-linear interactions. To avoid overfitting, it starts with the maximum number of basis functions and prunes them using the generalized cross-validation criterion.

The best model, with a degree of 2 and 20 pruning terms, showed an RMSE of 0.1269, R-squared of 0.459, and MAE of 0.0960 on the training set. The most influential variable was mnf_flow, followed by brand_code.C and hyd_pressure3, indicating these factors are critical in predicting pH levels.
...
Resampling metrics (RMSE, R², and MAE) from model training are compared to test metrics to evaluate overfitting and generalization capabilities. Models are judged on predicted accuracy, robustness, and computational efficiency. The final decision is based on these criteria to produce a model that performs well under various circumstances.
